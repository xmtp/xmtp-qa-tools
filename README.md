...
- Gen Keys: Generate wallet keys and encryption keys for XMTP testing - [see section](./scripts/generateKeys.ts)
  - Creates `.env` file with `XMTP_WALLET_KEY`, `XMTP_DB_ENCRYPTION_KEY`, and `XMTP_ENV`
  - Usage: `yarn gen:keys`
- Revoke Installations: Revoke XMTP installations for a given inbox ID - [see section](./scripts/revokeInstallations.ts)
  - Can keep specific installations or revoke all except one
  - Usage: `yarn revoke <inbox-id> [installations-to-save]`
