...
flowchart LR
  ...
  subgraph customers["Customers"]
    elacity["Elacity, Hopscotch"]
    nodle["Nodle, Family (v2)"]
    halo["Halo (v2 dms)"]
    convos["Convos, Coinbase"]
    xmtpAgents["bots/agents"]
    converseBackend["Converse Backend"]
  ...
  napi --- nodesdk
  nodesdk --- xmtpAgents
  nodesdk --- converseBackend

  decentralNode["Decentralized Nodes"] -.- libxmtp

  %% Flutter SDK connections (hand-rolled implementation)
  swiftSDK -.- flutterSDK
  kotlinSDK -.- flutterSDK
  flutterSDK --- halo
  flutterSDK --- unstoppable

  %% React Native SDK connections
  reactNativeSDK --- convos
  napi -.- reactNativeSDK

  %% Styling for highlighted path:
  %% (libxmtp -> Napi -> Node SDK -> bots/agents)
  %% Edge indices (based on the order defined above):
  %% libxmtp --- nodeBinding is the 4th edge,
  %% nodeBinding --- nodesdk is the 12th edge,
  %% nodesdk --- xmtpAgents is the 13th edge.
  linkStyle 3,11,12 stroke:#f66,stroke-width:4px,stroke-dasharray: 5,5;

  %% Styling for highlighted nodes and links
  classDef highlightStroke fill:#fff,stroke:#f66,color:#000,stroke-width:4px;
  class centralNode,libxmtp,napi,nodesdk,convos,reactNativeSDK,xmtpAgents highlightStroke;
