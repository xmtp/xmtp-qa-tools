...
> Testing utilities for XMTP network

## Features

- **🤖 Worker Framework**: Multi-user and multi-device testing scenarios
- **📊 Performance Benchmarks**: Message delivery, sync timing, and network metrics
- **🔍 Functional Tests**: Comprehensive test suites for core XMTP functionality
- **🕷️ Browser Testing**: Playwright integration for web client testing
- **🐛 Bug Reproduction**: Dedicated suites for tracking and fixing issues
- **📈 Metrics & Monitoring**: Integration with DataDog for observability
- **🎯 Version Testing**: Support for testing with specific SDK versions
...
# Run functional tests with specific SDK versions
yarn cli test functional --versions 209,210

# Run a specific test
yarn cli test suites/functional/dms.test.ts

# Run with version constraints
yarn cli test suites/functional/dms.test.ts --versions 209,210
```
...
The framework supports the following SDK versions:
