...
def calculate_fork_scores(csv_file):
        ...
        for col in df.columns:
            ...
            if col == "ENABLED_OPS":
                ...
                for op in ops:
                    ...
                    if forks > 0:
                        fork_scores[op] += forks
                        print("  ENABLED_OPS -> '{}' | forked -> +{}".format(op, forks))
                    else:
                        fork_scores[op] -= 1
                ...
                try:
                    ...
                    if is_max and forks > 0:
                        fork_scores[col] += forks
                        print("  {}={} (MAX) | forked -> +{}".format(col, val, forks))
    ...
    print("\nFinal Fork Correlation Scores (Weighted by fork count):")
    print("--------------------------------------------------------")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python fork_score_weighted.py <csv_file>")
        sys.exit(1)
