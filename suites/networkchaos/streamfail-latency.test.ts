...
describe(testName, async () => {
  ...
  it("should simulate a host-to-node outbound latency and verify stream response degradation", async () => {
    ...
    console.log("[test] Applying extreme outbound latency to all nodes (20s)");
    node1.addLatency(20000);
    node2.addLatency(20000);
    ...
    expect(brokenCheck.allReceived).toBe(true);  // Adjust this if failures are expected
    expect(brokenCheck.orderPercentage).toBe(100);  // Adjust if disorder is expected
  });
});
